...
export default function App() {
  ...
  const [user, setUser] = useState<{username: string, name: string, role: Role, branch?: string} | null>(null);
  ...
  const handleLogin = async (username: string, password?: string, userType?: string) => {
    try {
      ...
      if (data.status === 'success') {
        ...
        setUser({ username: employeeId, name: fullName || employeeId, role, branch });
