...
const INITIAL_ROOMS = [
  ...
  { id: 'RM-204', name: 'Phòng Serene B4', type: 'Phòng 2 người', floor: '2', currentCount: 0, maxCount: 2, status: 'TRỐNG', branch: 'Sunrise Riverside' },
  { id: 'RM-302', name: 'Phòng Comfort C2', type: 'Phòng 8 người', floor: '3', currentCount: 0, maxCount: 0, status: 'BẢO TRÌ', branch: 'The Landmark View' },
];
...
export default function RoomManagement() {
  ...
  const [bedsData, setBedsData] = useState<Record<string, {bedId: string, price: string, status: string, note: string}[]>>({
    'RM-101': [
      { bedId: '1', price: '1.500.000', status: 'Đã thuê', note: 'Giường dưới' },
      { bedId: '2', price: '1.500.000', status: 'Trống', note: 'Giường trên' },
      { bedId: '3', price: '1.500.000', status: 'Đã thuê', note: 'Giường dưới' },
      { bedId: '4', price: '1.500.000', status: 'Trống', note: 'Giường trên' },
    ]
  });
  ...
  const handleOpenAddBed = () => {
    ...
    if (roomBeds.length >= selectedRoom.maxCount) {
       setDeleteBedError(`Hệ thống chặn thao tác: Phòng ${selectedRoom.name} đã đạt số lượng tối đa (${selectedRoom.maxCount} giường).`);
       return;
    }
    setBedFormData({
      bedId: (roomBeds.length + 1).toString(),
      price: '1.500.000',
      status: 'Trống',
  ...
  const handleDeleteBedClick = (bed: any) => {
    if (bed.status !== 'Trống') {
      setDeleteBedError(`Hệ thống chặn thao tác xóa: Giường ${bed.bedId} đang có người ở. Cần đảm bảo giường trống mới được phép xóa.`);
      return;
    }
  ...
  const handleOpenAdd = () => {
    setFormData({ id: '', name: '', type: 'Phòng 4 người', floor: '1', maxCount: 4, status: 'TRỐNG', branch: 'Homestay Central Park' });
    setErrorMsg('');
    setShowAdd(true);
  };
  ...
  const handleDeleteClick = (room: any) => {
    if (room.currentCount > 0) {
      setDeleteError(`Hệ thống chặn thao tác xóa: Phòng ${room.name} đang có người ở. Cần đảm bảo dữ liệu phòng trống mới được phép xóa.`);
      return;
    }
                 ...
                 <div className="p-6">
                    ...
                    <div className="space-y-4">
                       <div>
                          ...
                          <input type="text" value={bedFormData.bedId} onChange={e => setBedFormData({...bedFormData, bedId: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 focus:outline-none focus:bg-white focus:border-[#B7705F]" />
                       </div>
                       <div>
              ...
              <div className="p-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                       ...
                       <input type="text" value={formData.id} onChange={e => setFormData({...formData, id: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 focus:outline-none focus:bg-white focus:border-[#B7705F]" placeholder="VD: RM-105" />
                    </div>
                    <div>
